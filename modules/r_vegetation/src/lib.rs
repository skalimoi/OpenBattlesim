...
use image::Luma;
use insolation::calculate_actual_insolation;
use nalgebra::Vector3;
use orography::calculate_normal_map;
use probabilities::calculate_probabilities;
...
impl SimConfig {
    ...
    pub fn calculate_maps(
        ...
        std::fs::create_dir_all(format!("results/{map_name}")).unwrap();

        insolation_image
            .save(format!("results/{map_name}/{map_name}_{}daylight_hours_insolation_image.png", sun_config.daylight_hours))
            .unwrap();
        edaphic_image
            .save(format!("results/{map_name}/{map_name}_edaphic_image.png"))
            .unwrap();
        hydrology_image
            .save(format!("results/{map_name}/{map_name}_water_image.png"))
            .unwrap();

        std::fs::write(
            format!("results/{map_name}/{map_name}_normals.json"),
    ...
    pub fn calculate_probabilities(&self, map_name: &str, vegetation_name: &str, daylight_hours: i32) {
        let soil_ids_map = GreyscaleImage::new(
            ImageReader::open(&self.maps[map_name].texture_map_path)
                .unwrap()
                .decode()
                .unwrap()
                .into_luma8()
        ...
        let insolation_map = GreyscaleImage::new(
            ImageReader::open(format!(
                "results/{map_name}/{map_name}_{}daylight_hours_insolation_image.png",
                daylight_hours
            ))
            .unwrap()
            .decode()
        ...
        let edaphic_map = GreyscaleImage::new(
            ImageReader::open(format!("results/{map_name}/{map_name}_edaphic_image.png"))
                .unwrap()
                .decode()
                .unwrap()
                .into_luma16()
        ...
        let hydrology_map = GreyscaleImage::new(
            ImageReader::open(format!("results/{map_name}/{map_name}_water_image.png"))
                .unwrap()
                .decode()
                .unwrap()
                .into_luma16()
        ...
        probabilities_image
            .save(format!(
                "results/{map_name}/{map_name}_{vegetation_name}_{daylight_hours}daylight_hours_probability_image.png"
            ))
            .unwrap();
    }
}
