...
func _ready():
	...
	for i in boid_number:
		...
		var pos = Vector3(randf_range(10.0, 400.0), randf_range(10.0, 400.0), randf_range(10.0, 400.0))
		print("CREATING BOID NUMBER %d", [i])
...
func _process(delta):
	...
	var vel_data = rd.buffer_get_data(vel_storage_buffer)
	var i = 0
	for boid in $"../BoidBase".get_children():
		# check if indexes okay
		boid.set_linear_velocity(Vector3(vel_data[i * 3],vel_data[i * 3 + 1], vel_data[i * 3 + 2]))
		i += 1
